    ...
    while cap.isOpened():
        ...
        for result in results:
            ...
            for person_idx in masks[2]:
                ...
                frame = draw_box_with_label(frame, person_box, label)

            if show_boxes_var.get():
                for hat_idx in masks[1]:
                    hat_box = xyxy[hat_idx]
                    frame = draw_box_with_label(frame, hat_box, "Mũ")

                for shirt_idx in masks[0]:
                    shirt_box = xyxy[shirt_idx]
                    frame = draw_box_with_label(frame, shirt_box, "Áo")
...
image_label = ctk.CTkLabel(left_frame, text="", font=("Arial", 16), fg_color="gray")
image_label.pack(fill="both", expand=True, padx=10, pady=10)

loading_gif = Image.open("./images/loading.gif")
loading_gif = loading_gif.resize((50, 50))
loading_gif = ImageTk.PhotoImage(loading_gif)
loading_label = ctk.CTkLabel(left_frame, text="")
loading_label.pack(pady=20)

btn_select_image = ctk.CTkButton(right_frame, text="Chọn Ảnh", command=lambda: process_image(None), font=("Arial", 16), width=200)
btn_select_image.pack(pady=20)

btn_select_video = ctk.CTkButton(right_frame, text="Chọn Video", command=lambda: process_video(None), font=("Arial", 16), width=200)
